...
paths['2T1S'] = paths['scripts'] .. '\\2Take1Script_Revive'
...
local files = {
    ...
    ['Net_log'] = paths['Menu'] .. '\\net_event.log',
    ['Notif_log'] = paths['Menu'] .. '\\notification.log',
    ['Player_log'] = paths['Menu'] .. '\\player.log',
    ['Script_log'] = paths['Menu'] .. '\\script_event.log',
    ['Log_file'] = paths['2T1S'] .. '\\2Take1Script.log',
    ['Chat_log'] = paths['2T1S'] .. '\\Chat.log',
    ['Console_log'] = paths['2T1S'] .. '\\Console.log',
...
settings[0][#settings[0] + 1] = 'log_console'
settings['log_console'] = false

settings[0][#settings[0] + 1] = 'print_scriptlog'
settings['print_scriptlog'] = false

settings[0][#settings[0] + 1] = 'print_chatlog'
settings['print_chatlog'] = false

settings[0][#settings[0] + 1] = 'print_joining_players'
settings['print_joining_players'] = false
...
settings[0][#settings[0] + 1] = 'profanity_filter'
...
settings[0][#settings[0] + 1] = 'splayer_log'
settings['splayer_log'] = false
...
settings[0][#settings[0] + 1] = 'chat_russki_choice'
settings['chat_russki_choice'] = 0

settings[0][#settings[0] + 1] = 'chat_chong'
settings['chat_chong'] = false
...
settings[0][#settings[0] + 1] = 'aim_prot_crash'
settings['aim_prot_crash'] = false
...
settings[0][#settings[0] + 1] = 'bodyguards_count_number'
settings['bodyguards_count_number'] = 0
...
modder_flags[0][#modder_flags[0] + 1] = 'filter'
modder_flags['filter'] = {'Profanity-Filter-Bypass', nil}
...
local function l(text)
    if settings['log_script'] then
        if not text then
            return
        end
        local prefix = u.time_prefix()
        local prefix = prefix .. ' [2Take1Script] '
        text = prefix .. text
...
setup.main = function()
    ...
    if _2t1s then
        n('2Take1Script Revive already loaded, stopping.', nc.r)
        return
    end
    if not s.d_exists(paths['2T1S']) then
        n('2Take1Script_Revive folder not found...\nRedownload the script and make sure you got all files!', nc.r)
        return
    else
        if s.f_exists(files['EXT_file']) then
            ...
            if extension_file then
                ...
                _2t1sEXT = false
                n('ERROR Loading Script EXT, returning!', nc.r)
                return
            end
        else
            n('2Take1ScriptEXT.lua not found!\nMake sure you have all important files!', nc.r)
            return
        end
        if not s.d_exists(paths['Config']) then
            n('2Take1Script_Revive/Config folder not found...\nRedownload the script and make sure you got all files!', nc.r)
            return
        end
        if not s.d_exists(paths['CustomFiles']) then
            n('2Take1Script_Revive/CustomFiles folder not found...\nRedownload the script and make sure you got all files!', nc.r)
            return
        end
    end
        ...
        for value in io.lines(files['Config']) do
            if not string.find(value, ']', 1) and not string.find(value, 'version', 1) then
                ...
                if settings[index] ~= nil then
                    ...
                    n("I found an outdated settings entry and cant read it, save settings to overwrite it.", nc.y)
                    n('Setting: ' .. index .. ' is invalid. Delete this entry or save settings.', nc.y)
                end
            end
        end
        io.close(_file)
        ...
        for key in io.lines(files['Hotkeys']) do
            if string.find(key, 'version', 1) then
                ...
                if version ~= settings['version'] then
                    n('Old Hotkeys-File detected. Delete the file or use the overwrite in the Hotkey-Options!', nc.y)
                end
            end
            if not string.find(key, '#', 1) and not string.find(key, 'version', 1) then
                ...
                if key ~= 'none' and key ~= 'nil' then
                    if hotkeys[index] then
                        ...
                        n("Outdated Hotkeys entry found. Delete the file or use the overwrite in the Hotkey-Options!\nHotkey: '" .. key .. "' with action: '" .. index .. "' is invalid. Delete this entry.", nc.y)
                    end
                end
            end
        end
...
setup.overwrite_hotkeys = function()
    ...
    io.write('#PageDown\n')
    io.close(hk)
    n("Created 2Take1Hotkeys.ini file in folder '2Take1Script_Revive/Config'. Edit Hotkeys and reload Hotkeys.ini file.", nc.g)
end
...
o.heading = function()
    return player.get_player_heading(s.id())
end
o.coords = function()
    return s.gcoords(o.ped())
end
...
m.o = {}
m.add = {}
m.add.p = function(name, place, callback)
    return menu.add_feature(name, 'parent', place, callback)
end
m.add.t = function(name, place, callback)
    return menu.add_feature(name, 'toggle', place, callback)
end
m.add.a = function(name, place, callback)
    return menu.add_feature(name, 'action', place, callback)
end
m.add.u = function(name, f_type, place, callback)
    return menu.add_feature(name, f_type, place, callback)
end
m.add.pp = function(name, place, callback)
    return menu.add_player_feature(name, 'parent', place, callback)
end
m.add.pt = function(name, place, callback)
    return menu.add_player_feature(name, 'toggle', place, callback)
end
m.add.pa = function(name, place, callback)
    return menu.add_player_feature(name, 'action', place, callback)
end
m.add.pu = function(name, f_type, place, callback)
    return menu.add_player_feature(name, f_type, place, callback)
end
...
valid.i = function(i, include)
    ...
    if s.valid(i) then
        if (include or i ~= s.id()) and g.scid(i) ~= -1 then
            if
                include or
                    (settings['exclude_friends'] and not player.is_player_friend(i) or not settings['exclude_friends'])
             then
                return true
            end
        end
            ...
            if bool1 and bool2 then
                ...
                m.add.a('Add Player to Remember-Modder', history_parent, function()
                    if scid == g.scid(s.id()) or scid == -1 then
                        n('Choose valid Player.', nc.r)
                    else
                        u.write(s.o(files['Modders'], 'a'), scid .. ' ' .. name)
                        n('Modder ' .. name .. ' added to Remember-List.', nc.g)
                ...
                m.add.a('Copy Outfit', history_parent, function()
                        ...
                        if female == history.players[i]['is_female'] then
                            ...
                            n('Unluckily, you have the wrong gender!', nc.r)
                        end
                    end)
                ...
                m.add.a('Is ' .. name .. ' in the current lobby?', history_parent, function()
                        for y = 0, 31 do
                            if g.scid(y) == scid then
                                n(name .. ' is in your lobby!', nc.y)
                                return HANDLER_POP
                            end
                        end
                        n(name .. ' is ~h~NOT~h~ in your lobby!', nc.y)
                    end)
                        ...
                        elseif not modder_player[scid] then
                            ...
                            for y = 1, #flags_int do
                                if modder_player[scid][flags_int[y]] then
                                    ...
                                    n(name .. " had '" .. text .. "' as a Flag!", nc.y)
                                end
                            end
                        end
                end)
...
history.new_lobby = function(lobby_no)
    ...
    history.parents[lenght] =
        m.add.p('Lobby ' .. lobby_no, m.p['player_history'], function()
        history.add_players()
...
local se_ceo = {
    {'Ban', 0x50c72ec2, {0, 1, 5}},
    {'Dismiss', 0xed1bc159, {0, 1, 5}},
    {'Terminate', 0xed1bc159, {1, 1, 6}}
}
local shoot_entitys = {
...
event.add_event_listener('exit', function()
        l('\n2T1Script Revive got unloaded.\nCleaning up...')
        n('2T1Script Revive got unloaded... :(', nc.r)
...
local function c_model(h, water, isinvisible, isdown, f_change)
    if
        not m.t['bl_mdl_change'].on or f_change or
            (m.t['bl_mdl_change'].on and not ped.is_ped_in_any_vehicle(o.ped()) and
                (water and entity.is_entity_in_water(o.ped()) or (not water and not entity.is_entity_in_water(o.ped()))))
     then
        if isdown then
            tp(o.coords(), 1.5)
        end
        ...
        n('Model Change not possible!', nc.r)
    end
end
...
local function crash_pl(lobby, id)
    for i = 0, 31 do
        if lobby then
            l('Lobby Crash incoming!')
            s.script(-2043109205, i, {0, 0, 30583, 0, 0, 0, -328966, -1933210564, 0})
        else
            s.script(-2043109205, id, {0, 0, 30583, 0, 0, 0, -328966, -1933210564, 0})
            break
        end
    end
end
...
local function is_entitled(id, target, cmd)
    l('Detected Chat-Command!\n' .. g.name(id) .. ':' .. g.scid(id) .. 'is trying to perform ' .. cmd .. ' as a Chat-Command!')
    if m.t['chat_cmd_friends'] and player.is_player_friend(id) or id == s.id() or m.t['chat_cmd_all'] then
        ...
        if target then
            ...
            n('Performing ' .. cmd .. ' Command for Player: ' .. g.name(id) .. ' on: ' .. g.name(id), nc.y)
            return true, plid
        end
        if pl_id ~= -1 then
            if pl_id == s.id() or (player.is_player_friend(pl_id) and m.t['exclude_friends'].on and pl_id ~= s.id()) then
                n(g.name(id) .. ' tried to perform a Command on you or a friend!', nc.y)
                l('Blocked from Performing Command!')
                return false
            else
                l('User is entitled to perfrom Command! Executing...')
                n('Performing ' .. cmd .. ' Command for Player: ' .. g.name(id) .. ' on: ' .. g.name(pl_id), nc.y)
                return true, pl_id
            end
        end
    end
    ...
    for i = 1, #data do
        if data[i][1] ~= false then
            ...
            if not data[i][2][1] then
                pos =
                    v3(
                    s.random(data[i][2][2], data[i][2][3]),
                    s.random(data[i][2][4], data[i][2][5]),
                    s.random(data[i][2][6], data[i][2][7])
                )
            else
            ...
            entity.set_entity_rotation(
                entitys['bl_objects'][#entitys['bl_objects']],
                v3(data[i][3][1], data[i][3][2], data[i][3][3])
            )
            if data[i][4] then
                entity.freeze_entity(entitys['bl_objects'][#entitys['bl_objects']], true)
            end
            if data[i][5] then
                s.visible(entitys['bl_objects'][#entitys['bl_objects']], false)
            end
        else
            if data[i] then
...
local function spawn_custom_vehicle(data, place)
    ...
    for i = 2, #data do
        ...
        if
            m.t['use_own_veh'].on and i == 2 and entity.get_entity_model_hash(cur_veh) == data[i][1] or
                data[2][1] == 0 and i == 2 and m.t['use_own_veh'].on and cur_veh ~= 0
         then
        ...
        elseif data[2][1] == 0 and not m.t['use_own_veh'].on then
            ...
            n('No Vehicle found, get in a valid Vehicle', nc.r)
            s.navigate(true)
            return
        else
        ...
        if m.t['set_godmode'].on then
            s.god(temp_veh[i - 1], true)
        end
        if data[i][5] then
            s.visible(temp_veh[i - 1], false)
        end
        if data[i][13] then
...
local function _2t1sf()
    l('Loading Features...')
    if settings['2t1s_parent'] then
        m.p['parent'] = m.add.p('2T1Script Revive', 0).id
    end

    m.p['self'] = m.add.p('Player Options', m.p['parent'])
    m.p['self'].hidden = settings['self_hidden']
    m.p['self'] = m.p['self'].id

    m.t['random_clothes'] =
    m.add.u('Random Clothes', 'value_i', m.p['self'], function(f)
        if f.on then
            s.wait(math.floor(1000 / f.value))
            ped.set_ped_random_component_variation(o.ped())
        end
        settings['random_clothes'] = f.on
        return u.stop(f)
    end)
    m.t['random_clothes'].min = 1
    m.t['random_clothes'].max = 25
    m.t['random_clothes'].value = settings['random_clothes_value']
                    ...
                    if target ~= 0 then
                        if target == o.ped() then
                            ...
                            if settings['electrocute_player'] then
                                n('Electrocuting Player ' .. g.name(i) .. '!', nc.g)
                            ...
                            if settings['aim_prot_crash'] then
                                crash_pl(false, i)
                            end
                        end
                    end
    ...
    m.t['aim_prot_kill'] = m.add.t('Explode', m.p['aim_protection'], function(f)
    ...
    m.t['aim_prot_crash'] = m.add.t('Crash Player', m.p['aim_protection'], function(f)
        settings['aim_prot_crash'] = f.on
    end)
    m.t['aim_prot_crash'].on = settings['aim_prot_crash']
    ...
    m.t['bodyguards_formation'].min = 0
    m.t['bodyguards_formation'].max = 3
    m.t['bodyguards_formation'].value = settings['bodyguards_formation_type']

    m.t['bodyguards_count'] = m.add.u('Set Number', 'autoaction_value_i', m.p['bodyguards'], function(f)
        settings['bodyguards_count_number'] = f.value
    end)
    m.t['bodyguards_count'].min = 1
    m.t['bodyguards_count'].max = 7
    m.t['bodyguards_count'].value = settings['bodyguards_count_number']
    ...
    m.t['bodyguards'] = m.add.t('Enable Bodyguards', m.p['bodyguards'], function(f)
            if f.on then
                ...
                for i = 1, m.t['bodyguards_count'].value do
                    if not entitys['bodyguards'][i] or entity.is_entity_dead(entitys['bodyguards'][i]) then
                        ...
                        if m.t['bodyguards_god'].on then
                            ...
                            ped.set_ped_max_health(entitys['bodyguards'][i], m.t['bodyguards_health'].value)
                            ped.set_ped_health(entitys['bodyguards'][i], m.t['bodyguards_health'].value)
                        end
                    ...
                    if not entity.is_entity_dead(entitys['bodyguards'][i]) then
                        ...
                        ped.set_group_formation(ped_group, m.t['bodyguards_formation'].value)
                            ...
                            for x = 1, #bl_veh do
                                if m.t[bl_veh[x][1]].on and veh == bl_veh[x][2] then
                                    ...
                                    if not entity.get_entity_god_mode(veh) then
                                        ...
                                        n('Detected Blacklisted Vehicle ' .. bl_veh[x][1] .. ' from user: ' .. g.name(i) .. ', exploding it!', nc.y)
                                        entity.set_entity_velocity(veh, v3())
    ...
    for i = 1, #block_locations['mazebank'] do
        m.add.a(
            block_locations['mazebank'][i][1],
            m.p['bl_area_mazebank'],
            function()
    ...
    m.t['explode_lobby'] =
        ...
        function(f)
            if f.on then
                for i = 1, 5 do
                    s.explode(
                        v3(s.random(-2700, 2700), s.random(-3300, 7500), s.random(30, 90)),
                        f.value,
                        true,
                        false,
                        0,
                        ...
                        if valid.i(id) then
                            ...
                            for i = 1, #ext_data.begger_texts do
                                if string.find(text, ext_data.begger_texts[i], 1) then
                                    ...
                                    n('Detected ' .. g.name(id) .. ' begging for Money! Punishing Player...', nc.y)
                    ...
                    function(h)
                        ...
                        if m.t['cmd_trap'] and string.find(text, '!trap ', 1) then
                            ...
                            if entitled then
                                local pos = s.gcoords(s.ped(pl_id))
                                entity.set_entity_rotation(
                                    spawn.object(1125864094, v3(pos.x, pos.y, pos.z - 5)),
                                    v3(0, 90, 0)
                                )
                            end
                        end
                        ...
                        if m.t['cmd_vehicle'] and string.find(text, '!vehicle ', 1) then
                            ...
                            if entitled then
                                ...
                                if streaming.is_model_a_vehicle(hash) then
                                    ...
                                    local spawned_veh =
                                        spawn.vehicle(hash, OffsetCoords(s.gcoords(s.ped(id)), heading, 10), heading)
                            ...
                            if entitled then
                                ...
                                if veh ~= 0 then
                                    ...
                                    if not req.ctrl(veh, 5000) then
                                        n("Couldn't get control over vehicle, possibility that command did not work.", nc.r)
                                    end
                                    if toggle ~= nil then
                                        s.god(veh, toggle)
                                    end
    ...
    m.t['modify_veh_speed'].min = -500
    m.t['modify_veh_speed'].max = 1000
    m.t['modify_veh_speed'].mod = 25
    m.t['modify_veh_speed'].value = settings['modify_veh_speed']
    m.add.a(
        'Reset Modifies',
        m.p['lobby_vehicle'],
        function()
    ...
    m.add.a(
        ...
        function()
            ...
            if not se_c then
                n('Aborted sending Custom Event...', nc.r)
                return HANDLER_POP
            end
            while se_c_p ~= '#' do
            ...
            n('Sent Custom Script Event with Parameters to Players.', nc.g)
        end
    )

    for i = 1, #ext_data.custom_se do
        m.add.a(ext_data.custom_se[i][1], m.p['lobby_se_custom'], function()
            n('Sent Custom Script Event to Players.', nc.g)
            for y = 0, 31 do
                if valid.i(y) then
    ...
    m.add.a('Send all to Island', m.p['lobby_se'], function()
        send_se(true, -1479371259, {1, 1})
        n('Time for a beach party!', nc.g)
    end)
    ...
    for i = 1, #send_2_mission do
        m.add.a('Send to ' .. send_2_mission[i][1], m.p['lobby_send_2_mission'], function()
            send_se(true, -1147284669, send_2_mission[i][2])
            n('Sent Session to Mission', nc.g)
        end)
    end
    ...
    for i = 1, 3 do
        m.add.a(se_ceo[i][1], m.p['lobby_ceo'], function()
            ...
            n('Modified Players CEO', nc.g)
        end)
    end

    m.add.a('Block - Passive', m.p['lobby_se'], function()
        send_se(true, 1472357458, {1, 1})
        n('Blocked all Players from activating Passive.', nc.g)
    end)

    m.add.a('UN-Block - Passive', m.p['lobby_se'], function()
        send_se(true, 1472357458, {1, 0})
        n('UN-Blocked all Players from Passive.', nc.g)
    end)

    m.p['lobby_sms'] = m.add.p('Send SMSs to Lobby', m.p['lobby'], function()
        n('Players must have Voice-Chat enabled to recive SMS.', nc.y)
        if m.t['sms_spam'] then
            m.t['sms_spam'].value = settings['sms_spam_value']
            m.t['sms_spam'].on = settings['sms_spam']
        end
    end).id

    m.t['sms_spam'] = m.add.u('Spam SMS X times', 'value_i', m.p['lobby_sms'], function(f)
        settings['sms_spam_value'] = f.value
        settings['sms_spam'] = f.on
    end)
    m.t['sms_spam'].min = 25
    m.t['sms_spam'].max = 5000
    m.t['sms_spam'].mod = 25
    m.t['sms_spam'].value = settings['sms_spam_value']
    m.t['sms_spam'].on = settings['sms_spam']
    ...
    m.t['force_host'] =m.add.t('Kick Hosts until You become Host', m.p['lobby_malicious'], function(f)
        if f.on then
            ...
            if host ~= s.id() and g.scid(host) ~= -1 then
                l('You are not Host!\n'.. g.name(host) .. ':' .. g.scid(host) .. ' is Host!')
                n(g.name(host) .. ' is now Host!', nc.y)
                kick_pl(false, host)
            end
        end
        settings['force_host'] = f.on
                ...
                for i = 0, 31 do
                    if valid.i(i) then
                        ...
                        if player.is_player_female(i) then
                            if
                                ped.get_ped_drawable_variation(s.ped(i), 1) == 134 and
                                    (ped.get_ped_texture_variation(s.ped(i), 1) == 8 or
                                        ped.get_ped_texture_variation(s.ped(i), 1) == 9)
                             then
                                guilty = guilty + 1
                            end
                            if
                                ped.get_ped_drawable_variation(s.ped(i), 4) == 113 and
                                    (ped.get_ped_texture_variation(s.ped(i), 4) == 8 or
                                        ped.get_ped_texture_variation(s.ped(i), 4) == 9)
                             then
                                guilty = guilty + 1
                            end
                            if
                                ped.get_ped_drawable_variation(s.ped(i), 6) == 87 and
                                    (ped.get_ped_texture_variation(s.ped(i), 6) == 8 or
                                        ped.get_ped_texture_variation(s.ped(i), 6) == 9)
                             then
                                guilty = guilty + 1
                            end
                            if
                                ped.get_ped_drawable_variation(s.ped(i), 11) == 287 and
                                    (ped.get_ped_texture_variation(s.ped(i), 11) == 8 or
                                        ped.get_ped_texture_variation(s.ped(i), 11) == 9)
                             then
                                guilty = guilty + 1
                            end
                        else
                            ...
                            if
                                ped.get_ped_drawable_variation(s.ped(i), 4) == 106 and
                                    (ped.get_ped_texture_variation(s.ped(i), 4) == 8 or
                                        ped.get_ped_texture_variation(s.ped(i), 4) == 9)
                             then
                                guilty = guilty + 1
                            ...
                            if
                                ped.get_ped_drawable_variation(s.ped(i), 6) == 83 and
                                    (ped.get_ped_texture_variation(s.ped(i), 6) == 8 or
                                        ped.get_ped_texture_variation(s.ped(i), 6) == 9)
                             then
                                guilty = guilty + 1
                            ...
                            if
                                ped.get_ped_drawable_variation(s.ped(i), 11) == 274 and
                                    (ped.get_ped_texture_variation(s.ped(i), 11) == 8 or
                                        ped.get_ped_texture_variation(s.ped(i), 11) == 9)
                             then
                                guilty = guilty + 1
                        ...
                        if guilty > 1 then
                            ...
                            n(g.name(i) .. ' is a useless alien! Punishing them!', nc.y)
    ...
    m.t['chat_russki'] = m.add.u('Punish if Russki Char is typed', "value_str", m.p['lobby_malicious'], function(f)
                ...
                if valid.i(id) then
                    ...
                    for i = 1, #ext_data.russki_chars do
                        if string.find(text, ext_data.russki_chars[i], 1) then
                            l("Detected '" .. ext_data.russki_chars[i] .. "' as a russki Char!\n Preparing to punish " .. g.name(id) .. '.')
                            n('Detected ' .. g.name(id) .. ' typing forbidden Russki! Punishing Player...', nc.y)
                            if m.t['chat_russki'].value == 0 then
                                kick_pl(false, id)
                            end
                            if m.t['chat_russki'].value == 1 then
                                crash_pl(false, id)
                            end
                            if m.t['chat_russki'].value == 2 then
                                local pos = s.gcoords(s.ped(id))
                                s.explode(pos, 8, false, true, 0, 0)
                            end
                            text = ''
                        end
                    end
        ...
        settings['chat_russki_choice'] = f.value
    end)
    m.t['chat_russki'].set_str_data(m.t['chat_russki'], {"Kick", "Crash", "Explode"})
    m.t['chat_russki'].value = settings['chat_russki_choice']
    m.t['chat_russki'].on = settings['chat_russki']

    m.t['chat_chong'] = m.add.t('Geo-Block China', m.p['lobby_malicious'], function(f)
        if f.on and not chat_events['chat_chong'] then
            chat_events['chat_chong'] = event.add_event_listener('chat', function(e)
                local id = e.player
                if valid.i(id) then
                    local text = e.body
                    for i = 1, #ext_data.chong_chars do
                        if string.find(text, ext_data.chong_chars[i], 1) then
                            l("Detected '" .. ext_data.chong_chars[i] .. "' as a chinese Char!\n Preparing to punish " .. g.name(id) .. '.')
                            n('Detected ' .. g.name(id) .. ' typing forbidden Chinese! Punishing Player...', nc.y)
                            crash_pl(false, id)
                            text = ''
                        end
                    end
        ...
        if not f.on and chat_events['chat_chong'] then
            event.remove_event_listener('chat', chat_events['chat_chong'])
            chat_events['chat_chong'] = nil
        end
        settings['chat_chong'] = f.on
    end)
    m.t['chat_chong'].on = settings['chat_chong']

    m.add.a('Kick Session', m.p['lobby_malicious'], function(f, i)
        kick_pl(true, i)
    end)

    m.add.a('Crash Session (SE)', m.p['lobby_malicious'], function(f, i)
        crash_pl(true, i)
    end)
    ...
    m.p['vehicle'] = m.add.p('Vehicle Options', m.p['parent']).id
    ...
    m.t['light_speed'] = m.add.u('Set Speed in Milliseconds', 'autoaction_value_i', m.p['vehicle_colors'], function(f)
        settings['veh_lights_speed'] = f.value
    end)
    m.t['light_speed'].min = 25
    m.t['light_speed'].max = 2500
    m.t['light_speed'].mod = 25
    m.t['light_speed'].value = settings['veh_lights_speed']
            ...
            if valid.vehicle(veh) then
                ...
                for i = 1, #neon_lights_rgb do
                    vehicle.set_vehicle_custom_primary_colour(
                        veh,
                        u.rgbtohex({neon_lights_rgb[i][1], neon_lights_rgb[i][2], neon_lights_rgb[i][3]}))
                    s.wait(m.t['light_speed'].value)
                end
            end
        end
    ...
    m.t['rainbow_pearlescent'] = m.add.t('Rainbow Pearlescent', m.p['rainbow_col'], function(f)
        if f.on then
            ...
            if valid.vehicle(veh) then
                req.ctrl(veh)
                for i = 1, #neon_lights_rgb do
                    vehicle.set_vehicle_custom_pearlescent_colour(
                        veh,
                        u.rgbtohex({neon_lights_rgb[i][1], neon_lights_rgb[i][2], neon_lights_rgb[i][3]}))
                    s.wait(m.t['light_speed'].value)
                end
    ...
    m.t['rainbow_neon'] = m.add.t('Rainbow Neon Lights', m.p['rainbow_col'], function(f)
        if f.on then
            ...
            if valid.vehicle(veh) then
                req.ctrl(veh)
                for i = 1, #neon_lights_rgb do
                    vehicle.set_vehicle_neon_lights_color(
                        veh,
                        u.rgbtohex({neon_lights_rgb[i][1], neon_lights_rgb[i][2], neon_lights_rgb[i][3]}))
                    s.wait(m.t['light_speed'].value)
                end
    ...
    m.t['synced_rainbow_smooth'] = m.add.t('Synced Smooth Rainbow', m.p['vehicle_colors'], function(f)
        if f.on then
            ...
            if valid.vehicle(veh) then
                local step
                step = math.floor((101 - (m.t['light_speed'].value / 25)) / 2)
                if step < 1 then
                    step = 1
                end
                for i = 0, 255, step do
    ...
    m.p['exp_beam'] = m.add.p('Explosive Beam on Horn', m.p['vehicle'])
    ...
    m.t['exp_beam'] = m.add.t('Enable Beam on Horn', m.p['exp_beam'], function(f)
        ...
        if f.on then
            local user = s.id()
            if g.scid(m.t['exp_beam_other'].value) ~= -1 then
                user = m.t['exp_beam_other'].value
            end
    ...
    m.t['exp_beam_type'] = m.add.u('Select Explosion', 'action_value_i', m.p['exp_beam'], function()
        settings['exp_beam_type'] = m.t['exp_beam_type'].value
        n('Beam Explosion Type 1: ' .. settings['exp_beam_type'], nc.g)
    end)
    m.t['exp_beam_type'].max = 74
    m.t['exp_beam_type'].min = 0
    m.t['exp_beam_type'].value = settings['exp_beam_type']

    m.t['exp_beam_type_2'] = m.add.u('Select Explosion 2', 'action_value_i', m.p['exp_beam'], function()
        settings['exp_beam_type_2'] = m.t['exp_beam_type_2'].value
        n('Beam Explosion Type 2: ' .. settings['exp_beam_type_2'], nc.g)
    end)
    m.t['exp_beam_type_2'].max = 74
    m.t['exp_beam_type_2'].min = 0
    m.t['exp_beam_type_2'].value = settings['exp_beam_type_2']

    m.t['exp_beam_radius'] = m.add.u('Select Scattering', 'action_value_i', m.p['exp_beam'], function()
        settings['exp_beam_radius'] = m.t['exp_beam_radius'].value
        n('Beam Radius: ' .. settings['exp_beam_radius'], nc.g)
    end)
    m.t['exp_beam_radius'].max = 10
    m.t['exp_beam_radius'].min = 1
    m.t['exp_beam_radius'].value = settings['exp_beam_radius']

    m.t['exp_beam_min'] = m.add.u('Select Min Range', 'action_value_i', m.p['exp_beam'], function()
        settings['exp_beam_min'] = m.t['exp_beam_min'].value
        n('Beam Min Range: ' .. settings['exp_beam_min'], nc.g)
    end)
    m.t['exp_beam_min'].max = 100
    m.t['exp_beam_min'].min = 10
    m.t['exp_beam_min'].value = settings['exp_beam_min']
    m.t['exp_beam_min'].mod = 5

    m.t['exp_beam_max'] = m.add.u('Select Max Range', 'action_value_i', m.p['exp_beam'], function()
        settings['exp_beam_max'] = m.t['exp_beam_max'].value
        n('Beam Max Range: ' .. settings['exp_beam_max'], nc.g)
    end)
    m.t['exp_beam_max'].max = 300
    m.t['exp_beam_max'].min = 100
    m.t['exp_beam_max'].value = settings['exp_beam_max']
    m.t['exp_beam_max'].mod = 5

    m.t['exp_beam_other'] =m.add.u('Enable Horn for Player', 'action_value_i', m.p['exp_beam'], function()
        if g.scid(m.t['exp_beam_other'].value) ~= -1 then
            n('Selected Player: ' .. g.name(m.t['exp_beam_other'].value), nc.g)
        else
            n('Not a valid Player.', nc.r)
        end
    end)
    m.t['exp_beam_other'].max = 31
    m.t['exp_beam_other'].min = -1
    m.t['exp_beam_other'].value = -1

    m.t['heli'] = m.add.u('Heli Blades Speed 0-100%', 'value_i', m.p['vehicle'], function(f)
        settings['heli'] = f.on
        settings['heli_i'] = f.value
        local veh = s.vehicle(o.ped())
        if f.on then
            if valid.vehicle(veh) then
                req.ctrl(veh)
                local speed = f.value / 100
    ...
    m.t['heli'].min = 0
    m.t['heli'].mod = 5
    m.t['heli'].value = settings['heli_i']
    ...
    m.t['sel_boost_speed'] = m.add.u('Boost Vehicle', 'value_i', m.p['vehicle'], function(f)
        ...
        settings['sel_boost_speed_speed'] = f.value
        local veh = s.vehicle(o.ped())
        if f.on then
            if valid.vehicle(veh) then
                req.ctrl(veh)
    ...
    m.t['sel_boost_speed'].max = 50000
    m.t['sel_boost_speed'].min = 0
    m.t['sel_boost_speed'].mod = 50
    m.t['sel_boost_speed'].value = settings['sel_boost_speed_speed']
    ...
    m.t['speedometer'] = m.add.u('License Plate Speedometer', 'value_i', m.p['vehicle'], function(f)
        ...
        settings['speedometer_type'] = f.value
        spm_1 = f.value
        if f.on then
            local veh = s.vehicle(o.ped())
            if valid.vehicle(veh) then
                if spm_1 ~= spm_2 then
                    n('Displaying Speed now with Unit:\n' .. ext_data.speedometer_units[f.value][3], nc.g)
                end
                local sp = entity.get_entity_speed(veh) * ext_data.speedometer_units[f.value][2]
                if sp < 10 and sp > 0.01 then
                    sp = string.format('%.2f', sp)
                elseif sp >= 10 and sp < 100 then
                    sp = string.format('%.1f', sp)
                elseif sp < 0.01 and f.value == 7 then
                    sp = string.format('%.5f', sp)
                else
                    sp = math.floor(sp)
                end
                req.ctrl(veh)
                vehicle.set_vehicle_number_plate_text(veh, tostring(sp) .. ext_data.speedometer_units[f.value][1])
            end
        end
        spm_2 = f.value
        return u.stop(f)
    end)
    m.t['speedometer'].max = #ext_data.speedometer_units
    m.t['speedometer'].min = 1
    m.t['speedometer'].value = settings['speedometer_type']
    ...
    m.p['custom_veh'] = m.add.p('Custom Vehicles', m.p['vehicle'])
    ...
    m.t['moveablerobot'] =m.add.t('Enable Robot', m.p['moveablerobot'], function(f)
        if f.on then
            if robot_objects['tampa'] == nil then
                ...
                if spawn_it then
                    ...
                    if not settings['disable_tampa_notify'] then
                        n('To get the best experience, upgrade the Tampa to use the Double Controllable Minigun!\nYou can disable this message in Options.', nc.y)
                    end
                end
            end
            if robot_objects['ppdump'] == nil then
                ...
                s.attach(
                    robot_objects['ppdump'],
                    robot_objects['tampa'],
                    0,
                    v3(0, 0, 12.5),
                    v3(),
                    true,
                    settings['robot_collision'],
                    false,
                    2,
                    true)
            end
    ...
    m.t['robot_shoot'] = m.add.t('Controllable Blasts', m.p['moveablerobot'], function(f)
            if f.on then
                ...
                dir = dir * 1000
                pos = pos + dir
                local hit, hitpos, hitsurf, hash, ent =
                    worldprobe.raycast((OffsetCoords(o.coords(), o.heading(), 2) + v3(0, 0, 4)), pos, -1, 0)
                while not hit do
                    pos = o.coords()
    ...
    m.t['robot_collision'] =m.add.t('Collision', m.p['moveablerobot'], function(f)
            ...
            if s.vehicle(o.ped()) == robot_objects['tampa'] then
                n('Re-enable Robot to take effect of collision!', nc.y)
            end
        end)
    ...
    m.t['rocket_propulsion'] = m.add.t('Rocket Propulsion (Visual)', m.p['moveablerobot'], function(f)
            if f.on and robot_objects['body'] then
                ...
                if robot_objects['spinning_middle2'] == nil then
                    ...
                    s.attach(
                        robot_objects['spinning_middle2'],
                        robot_objects['spinning_1'],
                        0,
                        v3(0, 0, 1.5),
                        v3(0, 0, 0),
                ...
                if robot_objects['spinning_middle3'] == nil then
                    ...
                    s.attach(
                        robot_objects['spinning_middle3'],
                        robot_objects['spinning_1'],
                        0,
                        v3(0, 0, 3),
                        v3(0, 0, 0),
            ...
            if #entitys['preview_veh'] > 0 and f.on then
                ...
                if not config_preview then
                    for i = 1, #entitys['preview_veh'] do
                        entity.set_entity_no_collsion_entity(entitys['preview_veh'][i], o.ped(), true)
                    end
                    config_preview = true
                end
    ...
    m.add.a('Delete Custom Vehicles', m.p['custom_spawner'], function()
            ...
            entitys['preview_veh'] = {}
            config_preview = false
            n('Cleared Custom Vehicles.', nc.g)
        end)
    for i = 1, #ext_data.custom_vehicles do
        m.add.a(
            ext_data.custom_vehicles[i][1],
            m.p['custom_spawner'],
            function()
    ...
    m.p['bac'] = m.add.p('Animal Model Changer', m.p['parent'], function()
        if m.t['revert_outfit'].on then
    ...
    m.add.a('Bigfoot',m.p['bac_ga'], function()
        c_model(0x61D4C771, nil, true, nil, true)
    end)

    m.add.a('Bigfoot 2', m.p['bac_ga'], function()
        c_model(0xAD340F5A, nil, true, nil, true)
    end)

    m.add.a('Boar', m.p['bac_ga'], function()
        c_model(0xCE5FF074)
    end)

    m.add.a('Cat', m.p['bac_ga'], function()
        c_model(0x573201B8)
    end)

    m.add.a('Chimp', m.p['bac_ga'], function()
        c_model(0xA8683715, nil, nil, true)
    end)

    m.add.a('Chop', m.p['bac_ga'], function()
        c_model(0x14EC17EA, nil, true)
    end)

    m.add.a('Cow', m.p['bac_ga'], function()
        c_model(0xFCFA9E1E)
    end)

    m.add.a('Coyote', m.p['bac_ga'], function()
        c_model(0x644AC75E)
    end)

    m.add.a('Deer', m.p['bac_ga'], function()
        c_model(0xD86B5A95)
    end)

    m.add.a('German Shepherd', m.p['bac_ga'], function()
        c_model(0x431FC24C, nil, true)
    end)

    m.add.a('Hen', m.p['bac_ga'], function()
        c_model(0x6AF51FAF)
    end)

    m.add.a('Husky', m.p['bac_ga'], function()
        c_model(0x4E8F95A2, nil, true)
    end)

    m.add.a('Mountain Lion', m.p['bac_ga'], function()
        c_model(0x1250D7BA, nil, true)
    end)

    m.add.a('Panther', m.p['bac_ga'], function()
        c_model(0xE71D5E68, nil, true)
    end)

    m.add.a('Pig', m.p['bac_ga'], function()
        c_model(0xB11BAB56)
    end)

    m.add.a('Poodle', m.p['bac_ga'], function()
        c_model(0x431D501C)
    end)

    m.add.a('Pug', m.p['bac_ga'], function()
        c_model(0x6D362854)
    end)

    m.add.a('Rabbit', m.p['bac_ga'], function()
        c_model(0xDFB55C81)
    end)

    m.add.a('Rat', m.p['bac_ga'], function()
        c_model(0xC3B52966)
    end)

    m.add.a('Golden Retriever', m.p['bac_ga'], function()
        c_model(0x349F33E1, nil, true)
    end)

    m.add.a('Rhesus', m.p['bac_ga'], function()
        c_model(0xC2D06F53, nil, nil, true)
    end)

    m.add.a('Rottweiler', m.p['bac_ga'], function()
        c_model(0x9563221D, nil, true)
    end)

    m.add.a('Westy', m.p['bac_ga'], function()
        c_model(0xAD7844BB)
    end)


    m.p['bac_wa'] = m.add.p('Water Animals', m.p['bac'], function()
        n('Note that these Models will only work in Water!', nc.y)
    end).id

    m.add.a('Dolphin', m.p['bac_wa'], function()
        c_model(0x8BBAB455, true)
    end)

    m.add.a('Fish', m.p['bac_wa'], function()
        c_model(0x2FD800B7, true)
    end)

    m.add.a('Hammershark', m.p['bac_wa'], function()
        c_model(0x3C831724, true)
    end)

    m.add.a('Humpback', m.p['bac_wa'], function()
        c_model(0x471BE4B2, true)
    end)

    m.add.a('Killerwhale', m.p['bac_wa'], function()
        c_model(0x8D8AC8B9, true)
    end)

    m.add.a('Shark', m.p['bac_wa'], function()
        c_model(0x06C3F072, true, true)
    end)

    m.add.a('Stingray', m.p['bac_wa'], function()
        c_model(0xA148614D, true)
    end)


    m.p['bac_fa'] = m.add.p('Flying Animals', m.p['bac']).id

    m.add.a('Cormorant', m.p['bac_fa'], function()
        c_model(0x56E29962)
    end)

    m.add.a('Chickenhawk', m.p['bac_fa'], function()
        c_model(0xAAB71F62)
    end)

    m.add.a('Crow', m.p['bac_fa'], function()
        c_model(0x18012A9F)
    end)

    m.add.a('Pigeon', m.p['bac_fa'], function()
        c_model(0x06A20728)
    end)

    m.add.a('Seagull', m.p['bac_fa'], function()
        c_model(0xD3939DFD)
    end)
    ...
    m.add.a('Franklin', m.p['bac_sm'], function()
        c_model(0x9B22DBAF, nil, nil, nil, true)
    end)

    m.add.a('Michael', m.p['bac_sm'], function()
        c_model(0x0D7114C9, nil, nil, nil, true)
    end)

    m.add.a('Trevor', m.p['bac_sm'], function()
        c_model(0x9B810FA2, nil, nil, nil, true)
    end)

    m.add.a('MP Female', m.p['bac_sm'], function()
        c_model(0x9C9EFFD8, nil, true, nil, true)
    end)

    m.add.a('MP Male', m.p['bac_sm'], function()
        c_model(0x705E61F2, nil, true, nil, true)
    end)

    m.t['bl_mdl_change'] = m.add.t('Safe Model Change', m.p['bac'], function(f)
        settings['bl_mdl_change'] = f.on
    end)
    ...
    m.p['weapon'] = m.add.p('Weapon Options', m.p['parent']).id
            ...
            if f.on then
                ...
                for i = 1, #ext_data.weapons do
                    for ii = 2, #ext_data.weapons[i] do
                        if
                            not weapon.has_ped_got_weapon_component(
                                o.ped(),
                                ext_data.weapons[i][1],
                                ext_data.weapons[i][ii]
                            )
                         then
                            for iii = 2, #ext_data.weapons[i] do
                                weapon.give_weapon_component_to_ped(
                                    o.ped(),
                                    ext_data.weapons[i][1],
                                    ext_data.weapons[i][iii]
                                )
                            end
    ...
    m.t['flamethrower_scale'] =
        m.add.u(
        'Scale',
        'autoaction_value_i',
        m.p['flamethrower'],
        function()
            settings['flamethrower_scale'] = m.t['flamethrower_scale'].value
        end
    )
    m.t['flamethrower_scale'].min = 1
    m.t['flamethrower_scale'].max = 25
    m.t['flamethrower_scale'].value = settings['flamethrower_scale']
    m.t['flamethrower'] =
        m.add.t(
        'Flamethrower',
        m.p['flamethrower'],
                ...
                for i = 1, #shoot_entitys do
                    if settings[shoot_entitys[i][1]] and ped.is_ped_shooting(o.ped()) then
                        ...
                        entity.apply_force_to_entity(
                            entitys['shooting'][#entitys['shooting']],
                            3,
                            vectorV3.x,
                            vectorV3.y,
                            vectorV3.z,
    ...
    for i = 1, #shoot_entitys do
        m.t[shoot_entitys[i][1]] =
            m.add.t(
            'Shoot ' .. shoot_entitys[i][1],
            m.p['shoot'],
            function(f)
        ...
        function(f)
            if f.on then
                if ped.is_ped_shooting(o.ped()) then
                    local pl = player.get_entity_player_is_aiming_at(s.id())
                    if ped.is_ped_a_player(pl) then
                        n('Attached Demi-God on Player: ' .. g.name(pl), nc.y)
    ...
    m.t['model_gun_ext'] = m.add.t('Add Objects and Vehicles to Model Gun', m.p['model_gun'], function(f)
        settings['model_gun_ext'] = f.on
    end)
    ...
    if settings['2t1s_parent'] then
        m.p['pl_parent'] = m.add.pp('2T1Script Revive', 0).id
    end

    m.add.pt('Unmark Modder', m.p['pl_parent'], function(f, id)
        if f.on then
    ...
    m.add.pt('Waypoint Player', m.p['pl_parent'], function(f, pid)
    ...
    m.add.pa('TP to Player', m.p['pl_parent'], function(f, id)
        tp(s.gcoords(s.ped(id)), 3)
    end)

    m.add.pa('TP Players Vehicle to me', m.p['pl_parent'], function(f, id)
        local veh = s.vehicle(s.ped(id))
        req.ctrl(veh)
        entity.set_entity_velocity(veh, v3())
        s_coords(veh, o.coords())
    ...
    m.add.pa('Ragdoll Player', m.p['pl_parent'], function(f, id)
        local pos = s.gcoords(s.ped(id))
        s.explode(pos, 70, false, true, 0, 0)
    end)

    m.p['pl_vehicle'] = m.add.pp('Vehicle Options', m.p['pl_parent']).id

    m.add.pt('Remote Control Vehicle', m.p['pl_vehicle'], function(f, pid)
        local pf = menu.get_player_feature(f.id)
        if pid == s.id() then
            f.on = false
            n("Don't.\nJust don't do it on yourself.", nc.r)
            return HANDLER_POP
        end
        if f.on then
            ...
            if veh ~= 0 then
                ...
                n('Target is not in a Vehicle!', nc.r)
                return HANDLER_POP
            end
        end
        if remote_veh then
    ...
    m.add.pa('Repair Vehicle', m.p['pl_vehicle'], function(f, pid)
        local veh = s.vehicle(s.ped(pid))
        if veh ~= 0 then
            req.ctrl(veh)
            vehicle.set_vehicle_fixed(veh)
    ...
    f = m.add.pu("Vehicle Godmode", "action_value_str", m.p['pl_vehicle'], function(f, pid)
        local veh = s.vehicle(s.ped(pid))
        if f.value == 0 then
            if veh ~= 0 then
                req.ctrl(veh)
	...
	f:set_str_data({"Give", "Remove"})

    m.add.pa('Kick from Vehicle', m.p['pl_vehicle'], function(f, id)
        send_se(false, 0xc40f66ca, {1, 0},nil, nil, id)
    end)

    m.add.pa('Modify Speed (0-500)', m.p['pl_vehicle'], function(f, pid)
        local veh = s.vehicle(s.ped(pid))
        if veh ~= 0 then
            ...
            if speed < 0 or speed > 500 then
                n('Invalid Speed.', nc.r)
                return HANDLER_POP
            end
            n('Setting modified Speed.', nc.g)
            req.ctrl(veh)
    ...
    m.p['attach'] = m.add.pp('Attach Objects', m.p['pl_parent']).id
    m.add.pa(
        ...
        function(f, id)
            ...
            if aimentity ~= 0 then
                ...
                n('No Entity found. Aim @Entity to attach it to Player.', nc.r)
            end
        end)
    ...
    m.add.pa('Clear Entitys', m.p['attach'], function()
        ...
        entitys['attach_obj'] = {}
        n('Cleared all Attachment Entitys.', nc.g)
    end)

    for i = 1, #ext_data.custom_attachments do
        m.add.pa(
        ext_data.custom_attachments[i][1],
        m.p['attach'], function(f, id)
            attach_entity(ext_data.custom_attachments[i][2], id)
        end)
    end

    m.p['pl_lag_area'] = m.add.pp('Lag Area with Vehicles', m.p['pl_parent'], function()
        n('DANGEROUS! ONLY USE WITH CAUTION!', nc.r)
    end).id
    for i = 1, #ext_data.vehicle_lag_area do
        m.add.pa('Lag / Rain Area with ' .. ext_data.vehicle_lag_area[i][1], m.p['pl_lag_area'], function(f, id)
            lag_area(id, ext_data.vehicle_lag_area[i][2])
        end)
    end

    m.p['pl_event_logger'] = m.add.pp('Log Events', m.p['pl_parent']).id

    m.add.pt('Log Script Events', m.p['pl_event_logger'], function(f, id)
            if f.on then
                if player_script_hooks[id] == nil then
                    player_script_hooks[id] =
                        hook.register_script_event_hook(
                        function(source, target, params, count)
                            if source == id then
                                ...
                                if not s.f_exists(file) then
                                    n("Logging to folder '2Take1Script_Revive/Event-Logger/" .. uuid, nc.g)
                                    text =
    ...
    m.add.pa('Reset Script Event Log', m.p['pl_event_logger'], function(f, id)
            local scid = g.scid(id)
            local name = g.name(id)
            local uuid = tostring(scid) .. '-' .. name
            local file = paths['Event-Logger'] .. '\\' .. uuid .. '\\' .. 'Script-Events.log'
            if s.f_exists(file) then
                ...
                n('There was no log to reset.', nc.r)
            end
        end)

    m.add.pt('Log net_events', m.p['pl_event_logger'], function(f, id)
            if f.on then
                if player_net_hooks[id] == nil then
                    player_net_hooks[id] =
                        hook.register_net_event_hook(
                        function(source, target, eventId)
                            if source == id then
                                local scid = g.scid(id)
                                local name = g.name(id)
                                local uuid = tostring(scid) .. '-' .. name
    ...
    m.add.pa('Reset net_event Log', m.p['pl_event_logger'], function(f, id)
            local scid = g.scid(id)
            local name = g.name(id)
            local uuid = tostring(scid) .. '-' .. name
            local file = paths['Event-Logger'] .. '\\' .. uuid .. '\\' .. 'Net-Events.log'
    ...
    for i = 1, #ext_data.custom_se do
        m.add.pa(
            ...
            m.p['pl_se_custom'],
            function(f, id)
                n('Sent Custom Script Event to Player.', nc.g)
                for x = 1, #ext_data.custom_se[i][2] do
    ...
    m.add.pa('Block - Passive', m.p['pl_se'], function(f, id)
        send_se(false, 1472357458, {1, 1}, nil, nil, id)
        n('Blocked Player from activating Passive.', nc.g)
    end)

    m.add.pa('UN-Block - Passive',m.p['pl_se'], function(f, id)
        send_se(false, 1472357458, {2, 0}, nil, nil, id)
        n('UN-Blocked Player from Passive.', nc.g)
    end)

    m.p['pl_send_2_mission'] = m.add.pp('Send Player to Mission', m.p['pl_se']).id
    for i = 1, #send_2_mission do
        m.add.pa('Send to ' .. send_2_mission[i][1], m.p['pl_send_2_mission'], function(f, id)
            send_se(false, -1147284669, send_2_mission[i][2], nil, nil, id)
            n('Sent Player to Mission', nc.g)
        end)
    end
    
    m.p['pl_ceo'] = m.add.pp('CEO', m.p['pl_se']).id
    for i = 1, 3 do
        m.add.pa(se_ceo[i][1], m.p['pl_ceo'], function(f, id)
            send_se(false, se_ceo[i][2], se_ceo[i][3], se_ceo[i][4], se_ceo[i][5], id)
            n('Modified Players CEO', nc.g)
         end)
    end

    m.p['pl_assassins_peds'] = m.add.pp('Send PEDs (Assassins)', m.p['pl_parent']).id

    m.add.pa('Clear PEDs', m.p['pl_assassins_peds'], function()
        clear(entitys['peds'])
        entitys['peds'] = {}
    end)
    for i = 1, #ext_data.ped_assassins do
        m.add.pa('Spawn ' .. ext_data.ped_assassins[i][1] .. ' (3x)', m.p['pl_assassins_peds'], function(f, id)
                ...
                for i = 1, 3 do
                    ...
                    for ii = 1, 26 do
                        ped.set_ped_ragdoll_blocking_flags(entitys['peds'][#entitys['peds']], ii)
                    end
    ...
    m.add.pa('Custom SMS input', m.p['pl_sms'], function(f, id)
    ...
    m.add.pa('Send their SCID & IP', m.p['pl_sms'], function(f, id)
    ...
    for i = 1, #ext_data.sms_texts do
        m.add.pa(ext_data.sms_texts[i], m.p['pl_sms'], function(f, id)
            player.send_player_sms(id, ext_data.sms_texts[i])
        end)
    end

    m.p['pl_misc'] = m.add.pp('Miscellaneous', m.p['pl_parent']).id
    ...
    m.add.pa(
        ...
        function(f, id)
            ...
            for i = 1, 25 do
                ...
                entity.apply_force_to_entity(
                    entitys['asteroids'][#entitys['asteroids']],
                    3,
                    0,
                    0,
                    force,
    ...
    m.add.pa(
        ...
        function(f, id)
            ...
            if ped.is_ped_in_any_vehicle(ped_id) then
                ped_id = s.vehicle(ped_id)
            else
                n('It works better, if target is in a Vehicle.', nc.y)
            end
            req.ctrl(ped_id)
    ...
    m.add.pa('Delete Vehicles', m.p['pl_lag_area'], function()
        l('Clearing Vehicles.')
        clear(entitys['lag_area'])
        entitys['lag_area'] = {}
        n('Cleared Vehicles.', nc.g)
    end)

    m.add.pa('Kick Player', m.p['pl_parent'], function(f, id)
        kick_pl(false, id)
    end)

    m.add.pa('Crash Player', m.p['pl_parent'], function(f, id)
        crash_pl(false, id)
    end)
    ...
    m.p['new_years_eve'] = m.add.u('New years eve', 'value_i', m.p['ptfx'], function(f)
        if f.on then
            ...
            s.bullet(
                pos,
                pos + v3(s.random(-25, 25), s.random(-25, 25), s.random(50, 75)),
                0,
                0x7F7497E5,
                o.ped(),
                true,
    ...
    m.p['new_years_eve'].max = 500
    m.p['new_years_eve'].mod = 25
    m.p['new_years_eve'].value = 100
    ...
    m.t['fire_fart'] =m.add.u('Fire Fart', 'action_value_i', m.p['ptfx'], function(f)
            settings['fire_fart'] = m.t['fire_fart'].value
            local veh = s.vehicle(o.ped())
            if veh ~= 0 then
                n('Fire Fart in a vehicle is too dangerous, get out!', nc.r)
            else
                ...
                local mult = f.value
                local fire = graphics.start_networked_ptfx_looped_on_entity(anim, o.ped(), v3(), v3(180, 0, 0), mult * 0.1)
    ...
    m.t['fire_fart'].max = 16
    m.t['fire_fart'].min = 4
    m.t['fire_fart'].value = settings['fire_fart']
    ...
    m.t['fire_ass'] = m.add.t('Fire Ass', m.p['ptfx'], function(f)
            ...
            if f.on then
                ...
                if ptfxs['fire_ass'] == nil then
                    ...
                    ptfxs['fire_ass'] = graphics.start_networked_ptfx_looped_on_entity(anim, o.ped(), v3(), v3(180, 0, 0), 0.333)
                end
                local pos = o.coords()
    ...
    m.p['modder'] = m.add.p('Modder Detection', m.p['parent'])
            ...
            for i = 0, 31 do
                if valid.i(i) then
                        ...
                        for ii = 1, #modder_scids do
                            if tostring(scid) == modder_scids[ii] then
                                ...
                                if not player.is_player_modder(i, -1) then
                                    n('Remembered ' .. g.name(i) .. ' as a Modder, remarking...', nc.y)
                    ...
                    if player.is_player_modder(i, -1) and not listed then
                        ...
                        n('Modder ' .. g.name(i) .. ' added to Remember-List.', nc.y)
    ...
    m.add.a('UN-Mark all Modders', m.p['modder'], function()
        for i = 0, 31 do
    ...
    m.t['speed_bypass'] = m.add.t('Max-Speed-Bypass', m.p['modder'], function(f)
        if f.on then
            for i = 0, 31 do
                local scid = g.scid(i)
                if
                    valid.modder(i) and player.get_player_health(i) ~= 0 and
                        interior.get_interior_from_entity(s.ped(i)) == 0
                 then
                    local maxspeed = 45
                    local id = s.ped(i)
                    if
                    ...
                    if speed > maxspeed then
                        n(g.name(i) .. ' bypassed Max-Speed-Limit of: ' .. maxspeed .. ' with a speed of: ' .. speed .. ' Marking them as a Modder...', nc.y)
                        l(g.name(i) .. ' bypassed Max-Speed-Limit of: ' .. maxspeed .. ' with a speed of: ' .. speed .. ' Marking them as a Modder...')
                        for ii = 0, 600 do
                            if ai.is_task_active(s.ped(i), ii) then
                                l('Current active Task: ' .. ii)
                            end
                        end
            ...
            local scid = g.scid(i)
                if valid.modder(i) then
                    ...
                    if string.len(name) < 6 or string.len(name) > 16 or not string.find(name, '^[%.%-%w_]+$') then
                        n(name .. ' has an illegal name! Marking them as a Modder...', nc.y)
                        l(name .. ' has an illegal name! Marking them as a Modder...')
            ...
            for i = 0, 31 do
                ...
                if valid.modder(i) then
                    ...
                    if scid < 10000 or scid > 222222222 then
                        local name = g.name(i)
                        n(name .. ' has a modded SCID! Marking them as a Modder...', nc.y)
                        l(name .. ' has a modded SCID! Marking them as a Modder...')
        ...
        if net_hook == nil then
            net_hook = hook.register_net_event_hook(function(source, target, eventId)
                if valid.modder(source) and target == s.id() then
                    ...
                    if guilty then
                        n(g.name(source) .. ' sent a Bad Net-Event: ' .. mapper.net.GetEventName(eventId) .. ' Marking them as a Modder!', nc.y)
        ...
        if not new_blame_kill_detection then
            new_blame_kill_detection = hook.register_net_event_hook(function(source, target, eventId)
                if eventId == 17 and valid.modder(source) and target == s.id() then
                    ...
                    if #guilty > 3 then
                        n("Detected '" .. g.name(source) .. "' blame killing other people!", nc.y)
        ...
        if bad_event_hook == nil then
            bad_event_hook =
                ...
                function(source, target, params, count)
                    ...
                    if guilty then
                        n("Detected '" .. g.name(source) .. "' sending a modded Script Event!", nc.y)
    ...
    m.t['profanity_filter'] = m.add.t('Profanity Filter Bypass', m.p['modder'], function(f)
        if f.on and not chat_events['profanity_filter'] then
            chat_events['profanity_filter'] = event.add_event_listener('chat', function(e)
                local id = e.player
                if valid.i(id) then
                    local text = e.body
                    for i = 1, #ext_data.profanity do
                        if string.find(text, ext_data.profanity[i], 1) then
                            n('Detected ' .. g.name(id) .. ' bypassing the profanity filter\n marking them as modder', nc.y)
                            player.set_player_as_modder(id, modder_flags['filter'][2])
                            text = ''
                        end
                    end
                end
        ...
        if not f.on and chat_events['profanity_filter'] then
            event.remove_event_listener('chat', chat_events['profanity_filter'])
            chat_events['profanity_filter'] = nil
        end
        settings['profanity_filter'] = f.on
    end)
    m.t['profanity_filter'].on = settings['profanity_filter']
        ...
        if f.on then
            ...
            if wp.x ~= 16000 then
                ...
                if wp:magnitude(v2pos) > 35 then
                    n('Detected Waypoint, teleporting...', nc.g)
                    local m = o.ped()
                    local v = s.vehicle(m)
                    if v ~= 0 then
                        m = v
    ...
    m.t['stealth'] = m.add.t("500K Stealth Recovery", m.p['misc'], function(f)
        if f.on then
            local pos = v2()
            local pos2 = v2()
            local pos3 = v2()
            ...
            ui.set_text_color(255, 206, 67, 255)
            ui.set_text_outline(true)
            ui.draw_text('Get Pranked LOL', pos)
            ui.set_text_scale(0.5)
            ui.set_text_centre(0)
            ui.set_text_color(255, 255, 255, 255)
            ui.draw_text('Go get out and do something with your life', pos2)
            ui.set_text_scale(0.5)
            ui.set_text_centre(0)
            ui.draw_text('You are fucking Pathetic', pos3)
            ui.draw_rect(.5, .5, 1, 1, 0, 0, 0, 255)
            ui.draw_rect(.5, .492, .52, .0019, 255, 255, 255, 255)
            ui.draw_rect(.5, .585, .52, .0019, 255, 255, 255, 255)
        end
    ...
    m.t['swap_seats'] = m.add.u('Swap Vehicle Seat', 'autoaction_value_i', m.p['misc'], function()
        ...
        if v ~= 0 then
            ped.set_ped_into_vehicle(o.ped(), v, m.t['swap_seats'].value)
        end
    end)
    m.t['swap_seats'].min = -1
    m.t['swap_seats'].value = -1
    m.t['swap_seats'].max = 15
    ...
    m.add.a('Fill Snacks and Armor', m.p['stats'], function()
        ...
        n('Filled Inventory with Snacks and Armor!', nc.g)
    ...
    m.add.a('Set KD (Kills / Death)', m.p['stats'], function()
        ...
        n('New KD set, to update the KD, earn a legit kill or death!', nc.g)
    end)
    ...
    m.add.a('Unlock Fast-Run Ability', m.p['stats'], function()
        ...
        n('New Ability set, change lobby to show effect!', nc.g)
    end)
    ...
    m.add.a(
        ...
        function()
            ...
            n('Casino Heist set up with random execution. If you dont like it, Reset Heist and try again!', nc.g)
        end
    )
    m.add.a(
        ...
        function()
            ...
            hashes = misc
            stat.set(hashes[4][2], true, hashes[4][4])
            n('Casino Heist set up with highest Payout. If you dont like it, Reset Heist and try again!', nc.g)
        end
    )
    ...
    m.o['chc_approach'].min = 1
    m.o['chc_approach'].max = 3
    m.o['chc_approach'].value = 1
    m.o['chc_hard'] =
        m.add.u(
        board1[2][1],
        'action_value_i',
    ...
    m.o['chc_content'] =
        ...
        m.p['chc_manual'],
        function(f)
            stat.set(board1[3][2], true, f.value)
        end
    )
    m.o['chc_content'].min = 0
    m.o['chc_content'].max = 3
    m.add.a(
        board1[4][1],
        m.p['chc_manual'],
        function()
    ...
    m.o['chc_content_2'] =
        ...
        function(f)
            local hashes = board2
            stat.set(hashes[1][2], true, f.value)
        end
    )
    m.o['chc_content_2'].min = 0
    m.o['chc_content_2'].max = 5
    m.o['chc_content_2'].value = 1
    ...
    m.o['chc_content_3'] =
        ...
        function(f)
            local hashes = board2
            stat.set(hashes[2][2], true, f.value)
        end
    )
    m.o['chc_content_3'].min = 0
    m.o['chc_content_3'].max = 5
    m.o['chc_content_3'].value = 1
    ...
    m.o['chc_content_4'] =
        ...
        function(f)
            local hashes = board2
            stat.set(hashes[3][2], true, f.value)
        end
    )
    m.o['chc_content_4'].min = 0
    m.o['chc_content_4'].max = 5
    m.o['chc_content_4'].value = 1
    m.o['chc_content_5'] =
        m.add.u(
        board2[4][1],
        'action_value_i',
        ...
        function(f)
            local hashes = board2
            stat.set(hashes[4][2], true, f.value)
        end
    )
    m.o['chc_content_5'].min = 0
    m.o['chc_content_5'].max = 1
    m.o['chc_content_6'] =
        m.add.u(
        board2[5][1],
        'action_value_i',
        ...
        function(f)
            local hashes = board2
            stat.set(hashes[5][2], true, f.value)
        end
    )
    m.o['chc_content_6'].min = 0
    m.o['chc_content_6'].max = 3
    m.add.a(
        board2[6][1],
        m.p['chc_manual'],
        function()
    ...
    m.o['chc_content_7'] =
        ...
        function(f)
            local hashes = board2
            stat.set(hashes[9][2], true, f.value)
        end
    )
    m.o['chc_content_7'].min = 0
    m.o['chc_content_7'].max = 12
    m.add.a(
        misc[4][1],
        m.p['chc_manual'],
        function()
    ...
    m.t['detect_lobby_change_for_history'] =
        m.add.t(
        'Detect lobby change',
        m.p['player_history'],
        function(f)
    ...
    m.p['utils'] = m.add.p('Utility', m.p['parent']).id


    m.p['outfits'] = m.add.p('Delete Custom Outfits', m.p['utils'], function()
            n('Attention!\nDeleting cant be reverted!', nc.y)
                    ...
                    m.add.a(
                        ...
                        function(f)
                            if s.f_exists(paths['outfits'] .. '\\' .. outfits[i]) then
                                ...
                                n('ERROR deleting the file, try again!', nc.r)
                                return HANDLER_POP
                            end
    ...
    m.add.a('Delete Entity from Aim', m.p['utils'], function()
        clear({player.get_entity_player_is_aiming_at(s.id())})
    end)


    m.add.a('Get input Hash Key', m.p['utils'], function()
            ...
            l('Hash: ' .. hash)
            n(string.format('%s %s', _input, hash), nc.g)
        end)


    m.add.a('Notify & Print String from file', m.p['utils'], function()
    ...
    m.t['print_info_from_entity'] = m.add.a('Print Info from Entity @Aim to file',  m.p['utils'], function()
            local aimentity = player.get_entity_player_is_aiming_at(s.id())
            ...
            if aimentity ~= 0 then
                while aimentity ~= 0 do
                    ...
                    if entity.is_entity_attached(aimentity) then
                        ...
                        l('\nAttached Entity found. Continue printing infos of Entity.')
                ...
                n('Printed Info about Entity to file.', nc.g)
            else
                n('Nothing found for Info-Printing.', nc.r)
            end
        end)


    m.p['console'] = m.add.p('Console', m.p['utils'], function()
        n('The lua Debug Console must be open for this!', nc.y)
    end).id


    m.t['print_scriptlog'] = m.add.t('Print 2T1Script Notifs to console', m.p['console'], function(f)
        settings['print_scriptlog'] = f.on
    end)
    m.t['print_scriptlog'].on = settings['print_scriptlog']
    ...
    m.t['print_chatlog'] = m.add.t('Print Chat to console', m.p['console'], function(f)
        ...
        if not f.on and chat_events['print_chatlog'] then
            event.remove_event_listener('chat', chat_events['print_chatlog'])
            chat_events['print_chatlog'] = nil
        end
        settings['print_chatlog'] = f.on
    end)
    ...
    m.t['print_joining_players'] = m.add.t('Print joining players to console', m.p['console'], function(f)
        if f.on and not player_events['print_joining_players'] then
            ...
            event.add_event_listener('player_join', function(e)
                pr('Player ' .. g.name(e.player) .. ' (' .. g.scid(e.player) .. ' / ' .. g.ip(e.player) .. ') joined the lobby', ' [2Take1]')
                if settings['splayer_log'] then
                    l('Player ' .. g.name(e.player) .. ' (' .. g.scid(e.player) .. ' / ' .. g.ip(e.player) .. ') joined the lobby')
                end
            end)
            if not f.on and player_events['print_joining_players'] then
                event.remove_event_listener('player_join', player_events['print_joining_players'])
                player_events['print_joining_players'] = nil
            end
            settings['print_joining_players'] = f.on
        end
    end)
    ...
    m.t['print_leaving_players'] = m.add.t('Print leaving players to console', m.p['console'], function(f)
        if f.on and not player_events['print_leaving_players1'] then
            ...
            event.add_event_listener('player_join', function(e)
                for i=0,31 do
                    if s.valid(i) then
                        pids[i] = player.get_player_name(i)
                    end
                end
            end)
            player_events['print_leaving_players2'] = 
            event.add_event_listener('player_leave', function(e)
                local prefix = u.time_prefix()
                if not s.valid(e.player) then 
                    if not pids[e.player] then return end
                    pr('Player ' ..  pids[e.player].. " left.", ' [2Take1]')
                    if settings['splayer_log'] then
                        l('Player ' .. g.name(e.player) .. ' (' .. g.scid(e.player) .. ' / ' .. g.ip(e.player) .. ') joined the lobby')
                    end
                    pids[e.player] = nil
        ...
        if not f.on and player_events['print_leaving_players1'] then
            event.remove_event_listener('player_join', player_events['print_leaving_players1'])
            event.remove_event_listener('player_leave', player_events['print_leaving_players2'])
            player_events['print_leaving_players1'] = nil
            player_events['print_leaving_players2'] = nil
        end
        settings['print_leaving_players'] = f.on
    end)
    ...
    m.t['log_modder_flags'] = m.add.t('Log Modder Flags', m.p['logs'], function(f)
    if f.on then
    ...
    m.t['splayer_log'] = m.add.t('Log Players', m.p['logs'], function(f)
        settings['splayer_log'] = f.on
    end)
    m.t['splayer_log'].on = settings['splayer_log']


    m.t['log_console'] = m.add.t('Log Console output', m.p['logs'], function(f)
        settings['log_console'] = f.on
    end)
    m.t['log_console'].on = settings['log_console']


    m.t['log_script'] = m.add.t('Enable 2T1Script logs', m.p['logs'], function(f)
        settings['log_script'] = f.on
    end)
    m.t['log_script'].on = settings['log_script']


    m.p['log-cleanup'] = m.add.p('Cleanup', m.p['logs']).id


    m.add.a('Clear All 2T1Script logs', m.p['log-cleanup'], function()
        u.write(s.o(files['Log_file'], 'w'), 'Cleared 2Take1Script.log')
        u.write(s.o(files['Chat_log'], 'w'), 'Cleared chat.log')
        u.write(s.o(files['Console_log'], 'w'), 'Cleared console.log')
        n('Cleared 2Take1Script.log', nc.g)
    end)


    m.add.a('Clear 2Take1Menu.log', m.p['log-cleanup'], function()
        u.write(s.o(files['Menu_log'], 'w'), '2Take1Menu.log cleared by 2Take1Script')
        n('Cleared 2Take1Menu.log', nc.g)
        l('Cleared 2Take1Menu.log')
    end)


    m.add.a('Clear 2Take1Prep.log', m.p['log-cleanup'], function()
        u.write(s.o(files['Prep'], 'w'), '2Take1Prep.log cleared by 2Take1Script')
        n('Cleared 2Take1Prep.log', nc.g)
        l('Cleared 2Take1Prep.log')
    end)


    m.add.a('Clear net_events.log', m.p['log-cleanup'], function()
        u.write(s.o(files['Net_log'], 'w'), 'net_event.log cleared by 2Take1Script')
        n('Cleared net_event.log', nc.g)
        l('Cleared net_event.log')
    end)


    m.add.a('Clear notification.log', m.p['log-cleanup'], function()
        u.write(s.o(files['Notif_log'], 'w'), 'notification.log cleared by 2Take1Script')
        n('Cleared notification.log', nc.g)
        l('Cleared notification.log')
    end)


    m.add.a('Clear player.log', m.p['log-cleanup'], function()
        u.write(s.o(files['Player_log'], 'w'), 'player.log cleared by 2Take1Script')
        n('Cleared player.log', nc.g)
        l('Cleared player.log')
    end)


    m.add.a('Clear script_event.log', m.p['log-cleanup'], function()
        u.write(s.o(files['Script_log'], 'w'), 'script_event.log cleared by 2Take1Script')
        n('Cleared script_event.log', nc.g)
        l('Cleared script_event.log')
    end)


    m.add.a('Clear All Menu Logs', m.p['log-cleanup'], function()
        u.write(s.o(files['Menu_log'], 'w'), '2Take1Menu.log cleared by 2Take1Script')
        u.write(s.o(files['Prep'], 'w'), '2Take1Prep.log cleared by 2Take1Script')
        u.write(s.o(files['Net_log'], 'w'), 'net_event.log cleared by 2Take1Script')
        u.write(s.o(files['Notif_log'], 'w'), 'notification.log cleared by 2Take1Script')
        u.write(s.o(files['Player_log'], 'w'), 'player.log cleared by 2Take1Script')
        u.write(s.o(files['Script_log'], 'w'), 'script_event.log cleared by 2Take1Script')
        n('All log files cleared.', nc.g)
        l('All log files cleared.')
        end)
                    ...
                    if hk_key ~= 'none' then
                        ...
                        if key:is_down() then
                            ...
                            if settings['hotkey_notification'] then
                                n(hk_key .. ":'" .. feat_name .. "' got pressed.", nc.y)
                            end
            ...
            if f.on then
                ...
                for i = 1, #mwh_key do
                    ...
                    if bool1 and bool2 then
                        ...
                        key:push_str(hk_key)
                        if key:is_down() then
                            n(hk_key .. ":'" .. hk_name .. "' got pressed.", nc.y)
                            s.wait(250)
                        end
                    end
                end
